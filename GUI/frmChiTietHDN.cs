...
namespace GUI
{
    public partial class frmChiTietHDN : Form
    {
        ...
        {
            dgvHoaDon.DataSource = ChiTietHDNDao.Instance.LoadCTSP(maHDN);
            ...
            DataTable dtHDB = HoaDonNhapDAO.Instance.TimKiemHDN_TYPE("MaHDN", maHDN);
            lblcontentMaHD.Text = maHDN;
            string maNCC = dtHDB.Rows[0]["maNCC"].ToString();
            lblContentTenNV.Text = dtHDB.Rows[0]["TenNV"].ToString();
            lblContentTenNCC.Text = dtHDB.Rows[0]["TenNCC"].ToString();
            lblContentTongTien.Text = dtHDB.Rows[0]["TongTien"].ToString() + " VNĐ";
            lblContentNgayLap.Text = dtHDB.Rows[0]["NgayNhap"].ToString();
            DataTable dtNCC = NhaCungCapDAO.Instance.TimKiemNCC_MaNCC(maNCC);
            lblContentSoDT.Text = dtNCC.Rows[0]["SDT"].ToString();
            lblContentDiaChi.Text = dtNCC.Rows[0]["DiaChi"].ToString();
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            exSheet.Range["A6"].Value = "Tên nhà cung cấp : " + lblContentTenNCC.Text;
            ...
            exSheet.Range["E" + (dong + 2).ToString()].Value = "Nhân viên lập:";
            exSheet.Range["E" + (dong + 2).ToString()].ColumnWidth = 20;
